...
internal fun AnimeSlide.toCacheJson(): JSONObject =
    JSONObject()
        .put("id", id)
        .put("title", title)
        ...
        .put("rankings", JSONArray().apply { rankings.forEach { put(it.toCacheJson()) } })
        .put("siteUrl", siteUrl)
...
internal fun JSONObject.toAnimeSlideOrNull(): AnimeSlide? {
    ...
    val parsed =
        buildList {
          for (i in 0 until array.length()) {
            array.optJSONObject(i)?.toMediaRankingOrNull()?.let { add(it) }
          }
        }
...
private fun JSONObject.parseRankingList(
  ...
  optJSONArray(arrayKey)?.let { array ->
    return buildList {
      for (i in 0 until array.length()) {
        array.optJSONObject(i)?.toMediaRankingOrNull(defaultType)?.let { add(it) }
      }
    }
  }
  optJSONObject(objectKey)?.toMediaRankingOrNull(defaultType)?.let { return listOf(it) }
...
private fun MediaRanking.toCacheJson(): JSONObject =
    JSONObject()
        .put("rank", rank)
        .put("type", type.name)
